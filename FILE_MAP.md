...
| `scripts/back-067-task-detail-reminder-smoke.mjs` | 352 | Headless Chrome/CDP smoke for task-detail reminder trigger at 390x844: verifies light/dark hit targets, horizontal options, selected value persistence, unclipped compact indicator and aria label | Run with `npm run smoke:back067-reminder` before changing task-detail reminder picker or indicator |

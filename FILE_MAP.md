...
| --- | ---: | --- | --- |
| `index.html` | 9307 | Telegram Mini App: HTML-экраны, JS-логика; CSS подключён из `styles.min.css` | Через `FILE_MAP_UI.md`, только нужный диапазон |
...
| `DEVELOPMENT_LOG.md` | 3622 | Канонический технический лог | Обновлять после значимых правок |
...
| `scripts/qa-press-feedback.js` | 39 | Shared QA press feedback installer for button glow, light haptics/vibration and textarea autoresize delegation previously held in inline `index.html` script | Read whole file before changing QA press feedback or keyboard enter hints |
...
| `scripts/vk-task-detail-edit-smoke.mjs` | 215 | Static smoke for VK task-detail edit path: extracts `vk.html` detail helpers and verifies visible detail summary, return hint, title/status/priority/deadline/person/description/originalMsg update payload plus local task state persistence | Run with `npm run smoke:vk-task-detail-edit` before changing VK task detail edit behavior |
...
| `scripts/vk-profile-parity-smoke.mjs` | 71 | Static smoke for VK profile parity: verifies account summary, identity summary, privacy/support links, security/session and AI-memory pointers, and local-only notification entry without touching payment/subscription | Run with `npm run smoke:vk-profile-parity` before changing VK profile structure |
| `scripts/vk-calendar-date-key-smoke.mjs` | 67 | Static smoke for VK calendar date keys: verifies ISO datetime and relative deadlines stay on intended days, calendar task dots render, all-deadline list renders, and selected-day task lists match normalized keys | Run with `npm run smoke:vk-calendar-date-key` before changing VK calendar date grouping |
...
| `scripts/vk-auth-session-smoke.mjs` | 144 | Static smoke for VK saved-session recovery: verifies `/auth/me` success keeps token, 401/403/explicit invalid token remove it, and 500/429/invalid JSON/network/timeout keep it with recovery UI | Run with `npm run smoke:vk-auth-session` before changing VK saved-token bootstrap |
| `scripts/vk-ai-chat-errors-smoke.mjs` | 134 | Static smoke for VK AI chat error handling: verifies 200/401/402/403/429/500 retry/network retry/malformed classes, x-token preservation, and redacted request-id diagnostics | Run with `npm run smoke:vk-ai-chat-errors` before changing VK `/anthropic` chat calls |
| `scripts/vk-ai-chat-main-parity-smoke.mjs` | 112 | Static smoke for VK AI chat parity: verifies chat-created tasks preserve originalMsg/description, `<create_task>` saves through existing save-task, and `<task_actions>` edit uses existing update-task | Run with `npm run smoke:vk-ai-chat-parity` before changing VK AI structured task tags |

...
| `vk.html` | 1630 | VK Mini App: отдельная версия без Telegram SDK | Через `FILE_MAP_UI.md`, только нужный диапазон |
...
| `scripts/back-067-task-detail-reminder-smoke.mjs` | 301 | Headless Chrome/CDP smoke for task-detail reminder at 390x844 in dark and light themes: verifies 44x44 trigger/options, horizontal text, real hit ownership via `elementFromPoint`, popover open/close, option selection, and form value persistence | Run with `npm run smoke:back067-reminder` before changing task-detail reminder picker or surrounding tag/hero stacking |

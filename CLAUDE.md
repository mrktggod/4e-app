...
### 4. Git
- Push, merge в `main`, `pull --rebase` и force push не выполнять автоматически.
- Работать через GitHub Desktop с явным подтверждением Алексея или Юрия.
- Перед началом ручной синхронизации: выбрать ветку, нажать `Fetch origin`, затем `Pull` только если GitHub Desktop предлагает безопасное обновление.
- Если есть незакоммиченные изменения или конфликт — остановиться и отдельно описать человеку риск.
- Исключения из Git-правил возможны только после явного согласования: указать правило, причину, риск и кто подтвердил.
...
0. Напомни пользователю проверить ветку и синхронизацию в GitHub Desktop. Не выполнять `pull --rebase`, push или merge автоматически.
...
```powershell
# App:
# Коммит, push и merge делает человек через GitHub Desktop.
# main связан с GitHub Pages, поэтому push в main влияет на live.

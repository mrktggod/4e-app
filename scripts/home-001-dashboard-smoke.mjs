...
const smokeTasks = [
  ...
  {
    id: 'home-smoke-followup',
    text: 'Write follow-up notes for pilot testers',
    direction: 'outgoing',
    directionLabel: 'Work',
    person: 'Alex',
    priority: 'p3',
    createdAt: new Date(Date.now() - 12800000).toISOString()
  },
  {
    id: 'home-smoke-done',
...
async function runSmoke(ws, appUrl) {
  ...
  const expression = `(${async function smoke() {
    ...
    metrics.heroLogoRemoved = !document.querySelector('.dash-hero-orb');
    ...
    assert(metrics.heroLogoRemoved, 'dashboard Focus card should not retain the removed decorative logo');
    ...
    const doneMetricBaseFrame = getComputedStyle(doneMetric, '::before');
    ...
    assert(metrics.doneFilterVisual.rimTop === '0px' && metrics.doneFilterVisual.rimLeft === '0px' && metrics.doneFilterVisual.rimRight === '0px' && metrics.doneFilterVisual.rimBottom === '0px', 'selected metric glass glow should fill the rounded frame and originate from its lower edge');
  ...
  if (tempDir) {
    for (let attempt = 0; attempt < 5; attempt += 1) {
      try {
        ...
        if (attempt === 4) {
          console.warn(`home001 smoke cleanup skipped locked temp dir: ${error.code || error.message}`);
          break;
        }

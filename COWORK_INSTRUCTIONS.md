...
1. **GitHub Desktop / git log** — что задеплоили и какая ветка активна
```
...
- Не пушить автоматически. Коммит, push, PR и merge делает человек через GitHub Desktop после явного решения.
...
### GitHub Desktop и main
- `main` связан с GitHub Pages, поэтому изменения в `main` могут попасть в live.
- Не выполнять автоматически `pull --rebase`, push, merge или force push.
- Если нужна синхронизация, попросить Алексея / Юрия открыть GitHub Desktop, выбрать ветку, нажать `Fetch origin`, затем `Pull` только если Desktop предлагает безопасное обновление.
- Если есть незакоммиченные изменения, конфликт или непонятная ветка — остановиться и вынести решение человеку.
- Исключения из Git-правил возможны только после согласования: указать правило, причину, риск и кто подтвердил.
...
|-----------|---------|-------|
| Mini App | Через GitHub Desktop: commit → push ветки → PR/merge только после проверки | 1-2 мин после merge |

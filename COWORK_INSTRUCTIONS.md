...
├── pm/inbox/            ← Операционные входящие брифы
├── pm/outbox/           ← Операционные отчёты
└── docs/tasks/          ← задачи для Мимо и архив
```

Бизнес-документация (roadmap/backlog/bugs/strategy, team sync, work log, QA/release docs) переехала в приватный репозиторий `https://github.com/mrktggod/4pm`.
...
4. **Командный журнал** в приватном `4pm`
5. **Team Sync** в приватном `4pm` — что Алексей передал Юре и что Юра вернул
6. **Баги и QA** в приватном `4pm`

### После проверки:
- Обнови раздел "Известные проблемы" в DEVELOPMENT_LOG.md
- Добавь запись о сегодняшних изменениях в DEVELOPMENT_LOG.md
- Добавь краткую запись в приватный `4pm` work log (формат: задача / результат / коммит / статус)
...
- Если готовится коммит — проверь заголовок по приватному `4pm/shared/COMMIT_CONVENTION.md`
- Если Юрий пишет `Что там у Лехи?` или `Закрой задачу и синхронизируй`, сначала прочитай приватный `4pm` team sync и действуй по нему.
...
### Team Sync
- Team sync живёт в приватном `4pm` и остаётся коротким диспетчерским экраном команды, а не вторым backlog.
- Фраза Юрия `Что там у Лехи?` означает: прочитать приватный `4pm`, PM-доки и Git-состояние, затем дать короткий отчёт.
- Фраза `Закрой задачу и синхронизируй` означает: проверить изменения, сделать commit, push в рабочую ветку и обновить team sync в приватном `4pm`. Если push не прошёл — объяснить причину и дать простую ручную инструкцию.

...
| 3 | Семантический HTML — можно брать Codex | средний |
| 4 | Подготовка кода под PostgreSQL заранее — можно брать Codex без credentials | средний |
| 5 | `bottom-nav-v2` (в #home) и `global-nav` — два отдельных компонента, нужно держать синхронизированными | средний |
| 6 | ANTHROPIC_KEY в worker.js должен быть только PLACEHOLDER — не коммитить реальный ключ | высокий |
...
- Добавлены Codex-задачи, которые можно делать сейчас: семантический HTML и подготовка кода под PostgreSQL заранее.

...
**What changed:** Replaced the dashboard's inner task-lane scrolling and per-frame JavaScript block movement with one native scroll surface on `#home`. The focus card and header now move naturally with the page, while the compact metric row uses CSS sticky positioning. Tightened the mobile layout (less top space, metrics closer to the focus card, task cards closer to the metrics), use fixed visual masks attached to the metric row and lower menu instead of moving a clip boundary with the task list, render every active task on the dashboard rather than only the current-period subset, and restored the notification button as a glass control with the existing inline SVG bell instead of theme PNG artwork.

**Why:** Telegram's touch compositor was competing with the synthetic gesture animation, which produced visible shaking and a transient dark frame. A single scroll frame gives the platform full control of inertia and makes both upward and downward gestures continuous. The PWA cache version now changes with this release so Telegram cannot resume the prior dashboard shell.
...
**What changed:** the dashboard profile and notification controls now use the same 44px hit area in both Telegram themes. Their centres and the clearance before the Focus card are therefore identical. In both themes, a saved avatar uses `contain` over a glass backing so no side, top or bottom of the image is cropped. The redundant light-theme task-date label above the metrics row is hidden, while a fixed 24px gap remains between the Focus card and metrics. The three task metrics now filter the dashboard list in place; only Progress opens statistics. The selected metric now stays transparent black in the dark theme: a separate green gradient meets its lower rounded boundary and casts a deliberately small glow below the glass instead of tinting the full card. The rest of its border is deliberately neutral and quiet; its inherited glossy top frame is removed, and a newly claimed PWA worker reloads the open Telegram WebView once so released CSS is applied immediately.

**Test:** `npm run build:css`; `npm run smoke:home001` verifies Done, Tasks and Promises filter the visible dashboard rows without navigation, while Progress opens statistics, and asserts the selected metric has the active glass state plus its gradient lower rim meeting the metric boundary while the inherited top frame is absent; `npm run smoke:telegram-bottom-menu` verifies both themes use 44x44 header controls with matching insets and Focus-card gap, saved dashboard photos use `contain`, and the light task-date label is absent while the Focus-to-metrics gap is 24px; `npm run check:cp1251-mojibake`; `npm run check:portable-paths`; `git diff --check`.
...
**Что сделано:** Нижняя панель Telegram поднята до отступа 32 px и увеличена до 66 px. Боковые кнопки подняты внутри панели, центральная кнопка уменьшена до 58 px, а стеклянная подложка меню сохранена. На коротком Telegram-экране список задач поднят и уплотнён, поэтому третья карточка не скрывается под верхним краем меню. Главный экран больше не ограничивает список тремя задачами: все задачи выбранного периода отрисовываются и доступны прокруткой. Свайп влево и действие «Перенести» теперь сразу открывают системный выбор даты; для Telegram добавлен резервный способ открытия, если `showPicker()` отклонён WebView.

**Тест:** `npm run build:css`; `npm run smoke:telegram-bottom-menu` подтверждает отступ 32 px, положение всех трёх кнопок в пределах панели, видимость третьей карточки, доступность шестой карточки после прокрутки и открытие выбора даты свайпом; `npx playwright test autotests/tests/web/navigation-safe-area.spec.ts --project=mobile-chromium` → 2 passed.
...
**Test:** `git checkout feat/admin-tariff-api`, `git fetch origin`, `git pull --ff-only`, inbox first-line scan, backlog/roadmap whitelist scan, `node scripts/check-cp1251-mojibake.mjs`, `npm run check:portable-paths`, `npm run check:ui-architecture`, `git diff --check`.
...
### DESIGN-GLASS-001 package 2 profile blocked

**Что сделано:** `BRIEF-2026-07-24-55` переведен в `BLOCKED-DEPENDENCY`; код не менялся. Brief 55 требует 53 и 54 `DONE`, но оба остановлены по dependency gate.
...
### DESIGN-GLASS-001 package 3 chat blocked

**Что сделано:** `BRIEF-2026-07-24-58` переведен в `BLOCKED-DEPENDENCY`; код не менялся. Brief 58 требует packages 1/2 и 57 `DONE`, но package 2 и 57 заблокированы.
...
### Telegram dashboard scroll collapse

**Что сделано:** Прокрутка списка задач теперь плавно сворачивает верхнюю часть главного экрана: карточка «Фокус дня» и заголовок уходят вверх, а сводные блоки закрепляются сверху над задачами. Движение привязано к самому свайпу и использует только `transform` и прозрачность.

**Проверка:** `npm run build:css`; `npm run smoke:telegram-bottom-menu` проверяет полное исчезновение карточки фокуса, закрепление сводки у верхнего края и восстановление исходного состояния при возврате списка наверх.

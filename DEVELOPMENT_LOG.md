...
## 2026-06-26 — Confirmation overlay восстановления пароля (Codex)

**Что сделано:**
- Исправлен остаточный пустой экран после toast "ссылка отправлена" в сценарии восстановления пароля.
- Добавлен `#forgot-result-overlay` вне `#app`, чтобы логическое продолжение было видно даже если активный screen/контейнер схлопнулся.
- Success-состояние показывает безопасный текст без раскрытия существования пользователя: "Если такой email есть в аккаунтах, мы отправили ссылку".
- Добавлены действия "Вернуться ко входу" и "Отправить ещё раз".
- Обновлены `FILE_MAP.md`, `FILE_MAP_UI.md`, `pm/bugs.md` и `pm/qa-checklist.md`.

**Проверка кодировки:** 23 / 23 совпадений по `Войти|Задачи|Сегодня`; перед правкой создан backup `index.backup_20260626_0142.html`.

**Тест:** точечный JS-smoke на функциях `showForgotPassword()`, `doForgotPassword()`, `showForgotPasswordResult()`, `finishForgotPasswordFlow()`, `retryForgotPasswordFlow()` проверил: fake email оставляет активным `forgot-password`, скрывает nav и включает `forgot-result-overlay`; "Вернуться ко входу" открывает login; "Отправить ещё раз" возвращает форму.

...
**What changed:** the dashboard profile and notification controls now use the same 44px hit area in both Telegram themes. Their centres and the clearance before the Focus card are therefore identical. In both themes, a saved avatar uses `contain` over a glass backing so no side, top or bottom of the image is cropped. The redundant light-theme task-date label above the metrics row is hidden, while a fixed 24px gap remains between the Focus card and metrics. The three task metrics now filter the dashboard list in place; only Progress opens statistics. The selected metric now uses a luminous glass rim, inset highlight and low green glow based on the provided glass-frame reference.

**Test:** `npm run build:css`; `npm run smoke:home001` verifies Done, Tasks and Promises filter the visible dashboard rows without navigation, while Progress opens statistics, and asserts the selected metric has the active glass state; `npm run smoke:telegram-bottom-menu` verifies both themes use 44x44 header controls with matching insets and Focus-card gap, saved dashboard photos use `contain`, and the light task-date label is absent while the Focus-to-metrics gap is 24px; `npm run check:cp1251-mojibake`; `npm run check:portable-paths`; `git diff --check`.

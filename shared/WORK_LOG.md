...
**Задача:** Сохранить live QA Алексея по `NEW-006` / `NEW-008` и передать Юрию найденные calendar/nav-регрессии
**Результат:** Зафиксирован partial pass safe-area и keyboard-smoke на iPhone; заведены `BUG-2026-07-20-001` (P1: центральная кнопка после календаря открывает voice) и `BUG-2026-07-20-002` (P2: сырая ISO-дата), добавлены скрин и repo-handoff Юрию
**Коммит:** `docs(qa): record calendar navigation regression`
**Статус:** ✅ QA-результат сохранён и подготовлен к передаче
**Следующий шаг:** Юрий исправляет P1 в отдельной ветке и возвращает staging-ссылку для повторного iPhone-smoke

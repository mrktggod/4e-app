...
**Задача:** CAL-002 — календарный MVP внутри 4 (первый slice).
**Делал:** Codex
**Коммит:** pending
**Состояние:** ◐ Partial Done
**Комментарий:** В `.tmp-4e-app-publish/index.html` текущий экран `calendar` перестал быть только списком дедлайнов: для выбранного дня добавлен блок `Штаб дня` с запланированными задачами по времени, оценкой длительности, задачами без слота, свободными окнами 09:00–21:00 и явным сигналом перегруза дня. Это первый календарный planning layer поверх уже существующих задач; отдельным следующим слоем остаются редактирование time blocking, AI-объяснение переноса и внешний busy/free.

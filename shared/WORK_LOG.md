...
**Задача:** создать ТЗ для голосового разбора нескольких задач и спрогнозировать приоритет
**Результат:** Создано `SMART-014` с UX, AI-контрактом, batch/idempotency требованиями, privacy, метриками, QA и этапами. В backlog/roadmap задача отмечена как стратегический P1, но runtime поставлен после закрытой beta и стабильного `BACK-021`; эталонный ответ закреплён как regression для `SMART-009/014`.
**Коммит:** `docs(ai): add smart 014 voice planning spec`
**Статус:** ✅ ТЗ выполнено; реализация не начиналась
**Следующий шаг:** Запрос на короткий review сохранён в `pm/agent-inbox/codex-to-yuri-2026-07-20-smart-014-review.md` и передаётся Юрию через draft Pull Request; после ответа Алексей отдельно решает, утверждать ли ТЗ и когда начинать runtime.

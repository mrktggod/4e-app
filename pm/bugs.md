...
| BUG-2026-07-21-008 | При автологине кратко показывается экран входа / есть задержка перед приложением | Medium | P2 | New / needs repro | Codex | N/A | Со слов Алексея: при входе/логине есть небольшая задержка между автологином и попаданием в приложение; если авторизация уже есть, экран входа лучше не показывать вообще или заменить на быстрый splash/loading. Нужно воспроизвести на `https://4-ai-staging.pages.dev/`: открыть приложение с сохранённой сессией, замерить время до home и проверить, мелькает ли login screen. Возможное направление фикса: ранний auth bootstrap по token/session до показа login UI, либо neutral splash до результата `/auth/me`. |
| BUG-2026-07-21-007 | Кнопка подписки на staging не открывает страницу | Medium | P1 | New / needs repro | Codex | BACK-010 / BACK-040 | Со слов Алексея во время ручной payment-check: на `https://4-ai-staging.pages.dev/` кнопка подписки не работает и не открывает страницу. Не проверялся реальный платеж; это UI/payment-entry bug до provider step. Нужно воспроизвести: войти в staging web app, открыть тариф/paywall/Premium, нажать кнопку подписки, проверить console/network и ожидаемый переход/модалку. |
| BUG-2026-07-21-006 | Уведомления плохо видны и иногда не срабатывают | High | P1 | NEED-YURI | Codex / Yuri QA | BACK-064 | Со слов Алексея: уведомления есть, но их особо не видно, иногда они не срабатывают; нужно полностью проверить и добиться звука/вибрации/видимости. Автономный проход 2026-07-22 не менял runtime: DoD требует real Telegram/device delivery smoke, звук/вибрацию и проверку правильного пользователя. `BACK-055` закрывал action-feed карточки, но не real delivery/salience. Создан task `docs/tasks/BACK-064-notification-salience-delivery-audit.md`; связанный delivery smoke: `docs/tasks/BACK-017-notifications-delivery-smoke.md`. |
| BUG-2026-07-21-005 | Пропадает часть истории чата | High | P1 | New / needs repro | Codex | N/A | Со слов Алексея: "стерлась часть чата, пропала просто". Нужно уточнить поверхность: AI-чат `ask`, чат задачи или Telegram/group `chat-conv`. Первичный code read: AI-чат намеренно ограничен `ASK_HISTORY_MAX = 40` (`index.html:5909`) и `trimAskHistory()` режет `askHistory.slice(-ASK_HISTORY_MAX)` (`index.html:5921`), remote load тоже запрашивает `/ai/messages?limit=40` (`index.html:5954`); task chat грузится с `limit=40` (`index.html:5448`); group chat UI полностью перерисовывается ответом `/messages?chatId=...` (`index.html:8012`, `index.html:8031`). |

...
| NEW-008 | AI-чат: поле ввода перекрывается нижним меню | High | P1 | Ready for QA | Юрий + Codex | N/A | Code evidence exists in backlog: `--app-keyboard-offset` and `.ask-bar--keyboard-open` are implemented. 2026-07-25 fixed the open-keyboard transition edge case and full `npm run qa:prebeta` passed. Live checklist: `docs/tasks/NEW-008-chat-keyboard-live-smoke.md`. Needs Yuri real Telegram Mini App/mobile keyboard smoke before Done. |

...
| BUG-2026-07-25-016 | Telegram group bot: бот не отвечает и не фиксирует задачи в групповом чате | High | P1 | NEED-YURI | Codex + Claude/Yuri | BOT-GROUP-001 | No runtime change from app automation: `FILE_MAP_BOT.md` says `mrktggod/4e-bot` is separate/not connected, local `4e-bot-repo` is non-canon per AGENTS, and live Telegram group actions are explicitly prohibited for automation. Yuri must provide canonical bot repo access/current worktree or live group evidence. |
| BUG-2026-07-25-017 | Focus of Day: home и popup показывают несогласованные счётчики задач | Medium | P1 | Ready for QA / home001 green | Codex | HOME-FOCUS-COUNTERS-001 | Fixed in app: focus card and popup derive visible focus count from `getFocusTaskSummary()`. `npm run smoke:home001` passed with focus card count `3`, popup rows `3`, popup task metric `3`, and subtitle `2 горят, 3 задачи в фокусе`; old ambiguous `4 выделил главное` copy is guarded against. |
| BUG-2026-07-25-018 | Statistics: экран говорит `Нет активных задач`, хотя home показывает активные задачи | High | P1 | Ready for QA / home001 green | Codex | STATS-ACTIVE-001 | Fixed in app: when statistics has no outgoing active tasks but has incoming active promises, the active-task block now clarifies `Активных задач без обещаний нет. 1 обещание показано выше.` instead of generic `Нет активных задач`. `smoke:home001` verifies seeded active tasks are listed and incoming-only active data gets the clarified empty state. |

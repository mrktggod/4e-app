...
| --- | --- | --- | --- | --- | --- | --- | --- |
| BUG-2026-07-25-008 | Expired Premium: завершение задачи показывает общее `Ошибка сохранения` вместо понятного premium/paywall состояния | High | P1 | Ready for QA / auto smoke green | Codex | PREMIUM-UX-001 | Пользователь Миша тестировал с истёкшим Premium: при попытке завершить задачу UI показал только `Ошибка сохранения`. Ожидание: если backend возвращает premium/entitlement denial, интерфейс прямо объясняет, что действие недоступно без Premium, и ведёт на экран преимуществ/оплаты; не маскировать это как обычный save failure. Скрин: чат 2026-07-25, premium expired task detail. |
| BUG-2026-07-25-009 | Expired Premium: голосовой режим выглядит как зависание/поломка вместо явного premium gate | High | P1 | Ready for QA / auto smoke green | Codex | PREMIUM-UX-001 / NEW-020 | В голосовом режиме у пользователя без активного Premium экран остаётся в состоянии `Говори...`/pipeline, без понятного объяснения: нужно оплатить или это баг. Ожидание: voice entrypoint до записи проверяет доступ или корректно обрабатывает отказ и показывает понятный Premium-required state с переходом к преимуществам. |
| BUG-2026-07-25-010 | Voice consent: чекбокс согласия на биометрию слишком маленький и слабо заметный | Medium | P1 | Ready for QA / focused smoke green | Codex | BACK-003 / BACK-050 | На экране `Согласие на обработку голоса` пользователь с небольшим ухудшением зрения не понял, куда ставить галочку. Ожидание: checkbox/touch target минимум 44px, заметный контраст и понятное состояние checked/unchecked без изменения юридического текста. |
| BUG-2026-07-25-011 | Voice mode: кнопка `Отменить` и стрелка назад не выходят из режима | High | P1 | Ready for QA / focused smoke green | Codex | NEW-020 | В голосовом режиме пользователь не может выйти: `Отменить` и back arrow не срабатывают. Ожидание: оба контрола останавливают запись/распознавание, чистят transient state и возвращают на предыдущий безопасный экран. |
| BUG-2026-07-25-012 | AI/task copy: `недавно` противоречит `47 дней назад` | Medium | P2 | Ready for QA / focused smoke green | Codex | HOME-001 / SMART-004 | Fix: `getTaskCreatedTimestamp()` now reads explicit created fields plus legacy `date`, and home/focus meta uses `formatTaskCreatedMeta()` with neutral `поставлена без даты` fallback instead of `поставлена недавно`. Proof: `npm run smoke:relative-time-copy` verifies a 47-day-old task renders `поставлена 47 дней назад` and no `недавно`. |
| BUG-2026-07-25-013 | Home dashboard: правый край интерфейса обрезан на телефоне | High | P1 | Ready for QA / geometry smoke green | Codex | NEW-006 / HOME-001 | Current branch does not reproduce clipping: `smoke:home001` now checks exact left/right viewport bounds at 390/360/320px for header, hero, metrics, task list, show-all, first row and bottom nav. Raw 320px proof keeps all right edges <= viewport (`dash-bottom-nav` right 306/320, task list right 302/320). |
| BUG-2026-07-25-014 | AI chat: нет понятной кнопки голосового ввода внутри composer | Medium | P2 | Ready for QA / focused e2e green | Codex | CHAT-VOICE-001 | In `index.html:438` the AI chat composer now includes a visible microphone button wired to existing `openVoice()`; `styles/screens/voice.less:1065` keeps the shell at textarea + voice + send without overflow. Focused Playwright smoke `autotests/tests/web/chat-keyboard.spec.ts` passed 4/4 and proves the entrypoint is visible, at least 44x44, and opens the biometric consent gate on mobile. |
| BUG-2026-07-25-015 | AI chat: команда удалить задачи была выполнена как перенос всех активных задач в выполненные | Critical | P0 | NEED-CLAUDE | Codex + Claude review | AI-INTENT-DELETE-001 | Autonomous runner did not change runtime: this is P0 destructive/bulk task behavior, and source evidence points to AI chat prompt/tool contract around `index.html:6881` plus action execution around `index.html:6784-6785`. Needs Claude-reviewed narrow fix plan so delete intent cannot be mapped to `complete`; see `pm/outbox/REPORT-BRIEF-2026-07-25-68-ai-delete-intent-safety.md`. |
| BUG-2026-07-25-016 | Telegram group bot: бот не отвечает и не фиксирует задачи в групповом чате | High | P1 | Triaged / queued for night | Codex + Claude/Yuri | BOT-001 / SMART-004 | В Telegram-группе после удаления и повторного добавления `4 -- AI секретарь` бот не реагирует на сообщения и не фиксирует задачи вроде `Мне надо завтра встретиться с Женей`. Ожидание: бот отвечает/создаёт задачи и управляет ими по поддерживаемым group-chat сценариям. Если локального bot repo/live Telegram нет, ночной runner должен классифицировать `NEED-CLAUDE`/`NEED-YURI`, а не угадывать. |
| BUG-2026-07-25-017 | Focus of Day: home и popup показывают несогласованные счётчики задач | Medium | P1 | Triaged / queued for night | Codex | HOME-001 | После продления Premium home показывает `3 задачи требуют внимания`, подпись с другим числом и popup `0 горит, 3 ждут ответа, 4 выделил главное`, хотя список содержит 3 задачи. Ожидание: все счётчики и подписи фокуса строятся из одного источника и не противоречат друг другу. |
| BUG-2026-07-25-018 | Statistics: экран говорит `Нет активных задач`, хотя home показывает активные задачи | High | P1 | Triaged / queued for night | Codex | NEW-002 / HOME-001 | На рабочем аккаунте Миши home показывает 3 задачи, но экран статистики в периоде `Месяц задач` показывает `Нет активных задач`. Ожидание: statistics использует актуальные задачи/тот же источник данных или честно объясняет фильтр периода; не показывать empty state при наличии активных задач. |
| BUG-2026-07-25-019 | Calendar: снизу сразу видны задачи, но они не кликаются | High | P1 | Triaged / queued for night | Codex | CALENDAR-UI-001 / NEW-002 | В разделе календаря задачи видны сразу снизу, но строки/карточки не открываются по нажатию. Ожидание: календарные задачи должны быть либо явно интерактивными и открывать detail, либо не выглядеть как кликабельный список; hit area не должен перекрываться слоями календаря/nav. |
| BUG-2026-07-25-020 | Task detail glass layout откатился от согласованного вида: meta cards перекрывают title | High | P1 | Triaged / queued for night | Codex | DESIGN-GLASS-001 / BACK-044 | В текущем task detail светлого glass-режима title пересекается с правыми meta cards, layout выглядит как старый сломанный вариант. Эталонный вид был согласован ранее: сверху отдельные панели статуса и участников, затем широкая hero-карточка с actions/date/priority/status в одну строку, без перекрытий и с читаемым title/description. Ожидание: привести task detail к эталонному glass layout со второго скрина, не ломая mobile safe-area и existing task-detail smokes. |
| BUG-2026-07-25-001 | Staging mobile: внутри приложения виден фейковый статусбар `9:41`, сеть и батарея | Medium | P1 | Triaged | Codex | DESIGN-GLASS-001 | Ручная проверка Алексея 2026-07-25 на `https://4-ai-staging.pages.dev/`: сверху уже есть реальный статусбар телефона, а внутри app дополнительно показан дизайнерский/mock statusbar `9:41` + иконки сети/заряда. Ожидание: в live web/TMA не показывать mock statusbar; если он нужен для design-preview, держать только за preview/demo flag. Скрины: чат Codex Image #1/#6. |
| BUG-2026-07-25-002 | Staging mobile: нижнее меню/кнопка чата частично спрятаны, чат недоступен с телефона | High | P1 | Triaged | Codex | NEW-006 / NEW-008 | Ручная проверка Алексея 2026-07-25 на телефоне: на home снизу видна только часть меню, кнопки чата нет или она уходит под нижнюю панель браузера/host chrome. Ожидание: нижняя навигация и вход в чат всегда видны/нажимаемы с учётом safe-area и browser/TMA chrome. Скрин: Image #1. |
| BUG-2026-07-25-003 | Home dashboard dark: текст задачи сливается с фоном и уходит за границы карточки | High | P1 | Triaged | Codex | HOME-001 / DESIGN-GLASS-001 | Ручная проверка Алексея 2026-07-25: первая задача на главном экране в тёмной теме почти не читается, длинный русский текст выходит за границу карточки. Ожидание: контрастный текст, 1-2 строки с ellipsis/line clamp, без horizontal overflow. Скрин: Image #1. |
| BUG-2026-07-25-004 | Список задач из dashboard/statistics: экран белый в тёмной теме, меню видно где не должно, длинные названия вылезают | High | P1 | Triaged | Codex | NEW-002 / HOME-001 | Ручная проверка 2026-07-25: при переходе из dashboard в список/статистику на телефоне и desktop экран списка белый, хотя пользователь в тёмной теме; нижнее меню остаётся видимым на экране, где не должно; длинные названия активных задач уходят вправо за карточку. Ожидание: тема наследуется, bottom nav скрывается/резервируется по правилам экрана, titles/meta не создают horizontal overflow. Скрины: Image #2/#3. |
| BUG-2026-07-25-005 | AI-чат desktop: длинный ввод опускает поле и кнопку отправки ниже видимой области | High | P1 | Triaged | Codex | NEW-008 | Ручная проверка 2026-07-25 на desktop staging: короткое сообщение отправляется нормально, но при длинном тексте поле ввода растёт/скроллится так, что composer и send button уходят вниз за видимость. Ожидание: textarea имеет max-height, внутренний scroll и панель ввода остаётся закреплённой в viewport. Скрин: Image #4. |
| BUG-2026-07-25-006 | Task detail: кнопка уведомления/reminder в самой задаче не открывает popup | High | P1 | Triaged | Codex | BACK-064 | Ручная проверка 2026-07-25: базовые попапы работают, но именно popup уведомления в карточке задачи не открывается. Ожидание: кнопка колокольчика/reminder открывает выбор времени на mobile/desktop, клики не проваливаются сквозь слой. Это ручной red для хвоста `BUG-2026-07-22-001`. |
| BUG-2026-07-25-007 | Task detail desktop: hero title ломается по 2-4 буквы в строке, описание обрезается | High | P1 | Triaged | Codex | DESIGN-GLASS-001 | Ручная проверка 2026-07-25 на desktop staging: в detail hero длинный заголовок задачи превращается в узкую колонку (`тест / за- / дача / сег...`), описание снизу тоже обрезается; справа meta-cards забирают слишком много ширины. Ожидание: title/description получают достаточную ширину, line clamp и не превращаются в вертикальную колонку. Скрин: Image #5. |
| BUG-2026-07-23-001 | Telegram Mini App: на главной видна только одна задача, хотя активных задач больше | High | P1 | Ready for QA / local smoke green | Codex | HOME-001 | Root cause: главный экран рендерил только AI priority rows и всегда скрывал `#home-show-all-btn`, поэтому остальные активные задачи могли быть в `/tasks`, но не попадали в видимый список. Fix: UI нормализует `taskId`/`task_id`/`key` в `id`, добирает dashboard rows из всех активных задач и показывает `Смотреть все задачи`, когда активных задач больше видимых карточек. Proof: `npm run smoke:home001` с 4 активными fixture-задачами даёт `homeRows=3`, `showAllDisplay=flex`; encoding markers `111 / 111`. |
| BUG-2026-07-22-001 | На iPhone/TMA не нажимается выбор времени уведомления в карточке задачи | High | P1 | Reopened / manual staging red | Codex | BACK-064 | Исправлено в app: `index.html:462` больше не вкладывает hidden `<select>` внутрь reminder `<button>`; `select#detail-reminder` оставлен sibling для прежнего save/update path, а trigger получил 44x44 CSS px вместо 36x36. Raw proof: `npm run smoke:back067-reminder` на 390x844 открыл popover, выбрал `1hour`, обновил форму и закрыл popover. Но ручная проверка Алексея 2026-07-25 на staging снова красная: popup уведомления в задаче не открывается; заведён `BUG-2026-07-25-006`. Evidence: `docs/tasks/BUG-2026-07-22-task-detail-ios-regressions.md`; report `pm/outbox/REPORT-BRIEF-2026-07-22-31-task-reminder-time-ios.md`. |
...
| NEW-006 | Safe area: перекрытие нижним меню | High | P1 | Reopened / manual staging red | Юрий + Codex | N/A | Code evidence already exists in backlog: safe-area variables and bottom-nav reserve are implemented, plus headless 390x844 smoke passed. Manual staging check 2026-07-25 found bottom nav/chat entry partially hidden on phone; see `BUG-2026-07-25-002`. Live TMA checklist: `docs/tasks/NEW-006-tma-safe-area-live-smoke.md`. |
...
| NEW-008 | AI-чат: поле ввода перекрывается нижним меню | High | P1 | Reopened / manual staging red | Юрий + Codex | N/A | Code evidence exists in backlog: `--app-keyboard-offset` and `.ask-bar--keyboard-open` are implemented. 2026-07-25 fixed the open-keyboard transition edge case and full `npm run qa:prebeta` passed. Manual staging check later on 2026-07-25 found two live tails: chat entry hidden on phone and desktop long input pushes composer below viewport; see `BUG-2026-07-25-002` and `BUG-2026-07-25-005`. Live checklist: `docs/tasks/NEW-008-chat-keyboard-live-smoke.md`. |

...
| BUG-2026-07-05-003 | На первом экране после 22:00 показывается отрицательное время до конца дня | Medium | P2 | Triaged | Codex / Юрий | BACK-056 | Mobile QA 2026-07-05: Home показывает `2 задач горят · -1 ч до конца дня` |
...
| NEW-002 | Статистика: неактуальные данные / не открываются активные задачи | Medium | P2 | Triaged | Юрий + Codex | N/A | Реальные задачи и истории должны соответствовать текущему состоянию; пересчитать периоды |
| NEW-003 | Профиль: дублируется аватар | Low | P3 | Triaged | Юрий + Codex | N/A | На профиле не должно быть двух одинаковых аватаров |
| NEW-004 | Профиль: личные данные занимают слишком много места | Low | P3 | Triaged | Юрий + Codex | N/A | Сжать визуальные блоки и уменьшить «вес» секции личных данных |
| NEW-005 | Карточка задачи: заголовок/срок обрезаются | Medium | P2 | Triaged | Юрий + Codex | N/A | Исправить высоты и переносы, чтобы срок всегда считывался целиком |
...
| NEW-007 | Карточка задачи: блоки «Обсуждение/Совет/История» требуют переработки | Medium | P2 | Triaged | Юрий + Codex | N/A | В карточке нужна полная и непрерывная история действий по задаче |
...
| NEW-009 | AI-чат: нет быстрых действий голос/вложения | Medium | P2 | Triaged | Юрий + Codex | N/A | Вернуть и проверить быстрые действия внизу инпута |
| NEW-010 | Главный экран: кнопка «Завершить» непропорциональна | Low | P3 | Triaged | Юрий + Codex | N/A | Обновить визуальные размеры и touch target кнопки |
| NEW-011 | Главный экран: задачи объединены в общий контейнер | Medium | P2 | Triaged | Юрий + Codex | N/A | Вернуть читаемую структуру блоков для главного экрана |
| NEW-012 | Статистика: не отображаются завершенные задачи и история | Medium | P2 | Triaged | Юрий + Codex | N/A | Проверить выгрузку/фильтры статусов и историю выполнения |
| NEW-013 | Главный экран: перегружен «Фокус дня» | Low | P3 | Triaged | Юрий + Codex | N/A | Уменьшить визуальную перегрузку блока |
| NEW-014 | План дня: нет детальной сводки | Medium | P2 | Triaged | Юрий + Codex | N/A | Добавить отдельный экран/деталь задачи для плана дня |
| NEW-015 | План дня: отсутствуют метки направления/даты постановки/дедлайна | Medium | P2 | Triaged | Юрий + Codex | N/A | Расширить карточки план-потока минимум до 3 метаданных |
| NEW-016 | Фокус дня: текст наезжает на декоративную планету | Low | P3 | Triaged | Юрий + Codex | N/A | Исправить layout и отступы в зоне декора |

...
Для багов от внешних пользователей используем два слоя:

- `CLIENT-BUG-YYYY-MM-DD-NNN` — сырая заявка клиента из бота/чата.
- `BUG-YYYY-MM-DD-NNN` — подтверждённый баг после триажа.

Сырую клиентскую заявку нельзя сразу считать разработческой задачей. Сначала агент проверяет актуальность:

1. На какой версии/ссылке был баг.
2. Есть ли дубль в `pm/bugs.md`.
3. Воспроизводится ли на свежем staging/preview.
4. Не относится ли скриншот к старому дизайну.
5. Есть ли согласие на просмотр скриншота, если там могут быть личные данные.

Статусы клиентских заявок:

- `NEW` — только пришло.
- `NEEDS-REPRO` — не хватает шагов или не воспроизведено на свежей версии.
- `CONFIRMED` — воспроизведено, можно заводить обычный `BUG-*`.
- `DUPLICATE` — дубль существующего бага.
- `STALE` — похоже на старую сборку или уже исправлено.
- `CLOSED` — закрыто после проверки/ответа.

В подтверждённом баге обязательно писать `source: client`, чтобы команда видела, что это не внутренний smoke, а сигнал пользователя.
...
| BUG-2026-07-22-001 | На iPhone/TMA не нажимается выбор времени уведомления в карточке задачи | High | P1 | Ready for QA / headless mobile smoke green | Codex | BACK-064 | Исправлено в app: `index.html:462` больше не вкладывает hidden `<select>` внутрь reminder `<button>`; `select#detail-reminder` оставлен sibling для прежнего save/update path, а trigger получил 44x44 CSS px вместо 36x36. Raw proof: `npm run smoke:back067-reminder` на 390x844 открыл popover, выбрал `1hour`, обновил форму и закрыл popover. Real iPhone/TMA smoke всё ещё нужен перед notification delivery QA. Evidence: `docs/tasks/BUG-2026-07-22-task-detail-ios-regressions.md`; report `pm/outbox/REPORT-BRIEF-2026-07-22-31-task-reminder-time-ios.md`. |

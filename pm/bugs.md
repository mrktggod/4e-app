...
| BUG-2026-07-25-012 | AI/task copy: `недавно` противоречит `47 дней назад` | Medium | P2 | Triaged / queued for night | Codex | HOME-001 / SMART-004 | В карточке/AI-копирайтинге задача помечена как поставленная `недавно`, но рядом указан возраст `47 дней`. Ожидание: относительное время не должно противоречить фактическому возрасту; для старых задач использовать точную формулировку без `недавно`. |

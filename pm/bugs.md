...
**Решение:** исправить в рамках `BACK-056`: расчёт Home focus copy не должен использовать `22 - currentHour` без нижней границы. Минимальный фикс — `Math.max(0, 22 - hr)` плюс отдельная фраза для `0`; продуктово лучше заменить на human-friendly статус после 22:00.

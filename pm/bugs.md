...
| BUG-2026-07-25-019 | Calendar: снизу сразу видны задачи, но они не кликаются | High | P1 | Ready for QA / home001 green | Codex | CALENDAR-UI-001 | Fixed in app: all calendar deadline rows now use `data-cal-task-id`, role/button semantics, cursor/focus styling, and delegated click/Enter/Space routing to existing task detail. `smoke:home001` passed with `calendarTaskRows: 6` and a calendar row click opened `task-detail`. |

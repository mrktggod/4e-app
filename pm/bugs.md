...
| --- | --- | --- | --- | --- | --- | --- | --- |
| BUG-2026-07-05-002 | Профиль в mobile/desktop web выглядит неаккуратно: нет цельной сетки, съехали поля, бейджи и секции формы | Medium | P2 | Triaged | Юрий | BACK-043 | Скрины Алексея 2026-07-05; нужен responsive UI redesign без смены цветов и шрифта |
...
### BUG-2026-07-05-002 — Профиль в web-версии выглядит неаккуратно

**Дата:** 2026-07-05
**Версия/окружение:** веб-версия / mobile viewport и desktop viewport, профиль пользователя
**Severity:** Medium
**Priority:** P2
**Статус:** Triaged
**Метка:** UI / Profile / Responsive layout

**Почему существенно:** профиль — экран доверия и настройки аккаунта. Сейчас форма выглядит собранной небрежно и на mobile web, и на desktop web: нет цельной сетки, поля разной ширины спорят друг с другом, статусные бейджи живут как отдельные случайные элементы, а заголовки секций попадают в границы карточек. Это не блокирует сценарий, но снижает ощущение качества продукта перед закрытым тестом.

**Шаги воспроизведения:**
1. Открыть веб-версию приложения в mobile viewport и desktop viewport.
...
**Фактический результат:** на desktop viewport сетка ломается: поле телефона становится слишком узким, email обрезается, бейджи "Не подтверждён" и "Не привязан" занимают отдельные плашки и конфликтуют с input, строка Telegram визуально дублирует статус внутри поля и рядом с ним. На mobile viewport заголовки "Дата рождения" и "О себе" стоят слишком близко к предыдущим блокам; поле "О себе" и счётчик `0 / 200` не образуют аккуратную единую секцию; общий ритм отступов в форме неровный.

**Ожидаемый результат:** профиль выглядит как цельная, профессионально собранная форма на desktop и mobile: единая сетка, одинаковый вертикальный ритм, понятная иерархия блоков, статусы не конфликтуют с полями, textarea "О себе" и счётчик читаются как один блок. Цветовую схему и шрифт не менять; можно менять композицию, сетку, ширины, высоты, отступы, группировку полей и отображение статусов.

**Вложения/логи:** четыре скрина Алексея 2026-07-05 с красными стрелками на проблемные места в профиле: mobile web и desktop web.

**Решение:** `BACK-043` расширить до responsive UI redesign профиля. Юре нужно не точечно двигать один бейдж, а переработать форму как дизайнерский блок: задать сетку, правила для contact rows, статусы phone/email/telegram, заголовки секций, textarea "О себе", счётчик символов и расстояние до кнопки "Сохранить профиль" / нижней навигации.

**Проверка после фикса:**
1. Desktop web 1024px+: форма не разваливается на случайные колонки; поля и статусы имеют понятную сетку и не обрезают текст.
2. Mobile web 390px: строки phone/email/telegram выглядят ровно, бейджи статуса не налезают на поля и не выглядят случайно приклеенными.
3. Заголовки "Дата рождения" и "О себе" имеют нормальный верхний отступ и не прилипают к предыдущей карточке.
4. Поле "О себе" не перекрывается нижней навигацией; счётчик `0 / 200` стоит в понятном месте и не висит отдельно.
5. Кнопка "Сохранить профиль" доступна после прокрутки и не конфликтует с нижней навигацией.
6. Telegram WebView не получает регрессий на экране профиля.

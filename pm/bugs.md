...
| --- | --- | --- | --- | --- | --- | --- | --- |
| BUG-2026-07-14-001 | Staging web auth показывает «Нет соединения» после успешных /auth/register и /auth/login | High | P1 | In Progress | Codex | N/A | 2026-07-14 добран root cause по живому staging: это не CORS (`OPTIONS /auth/login` с `Origin: https://4-ai-staging.pages.dev` возвращает `204` + корректный `Access-Control-Allow-Origin`), а client-side crash после успешного auth-response. Сначала реальный alias `4-ai-staging.pages.dev` отдавал старый `index.html`, где `renderExtendedProfile()` вызывал голый `buildReferralLink()` и падал после `POST /auth/register|login`, маскируясь под toast `Нет соединения`. После staging deploy вскрылась и глубинная проблема: `scripts/auth.js` не экспортировал часть helper-глобалей (`processOAuthCallback`, `capturePendingReferralCode`, `getTelegramStartTokenFromLaunch`, `isVkMiniAppContext`, `getPendingReferralCode`, `clearPendingReferralCode`), хотя `index.html` и `auth-handlers.js` зовут их напрямую. Код и staging alias уже обновлены: `index.html` отвязан от скрытой глобали через `getProfileReferralLink()`, `auth.js` теперь экспортирует полный набор helper-ов, а alias `https://4-ai-staging.pages.dev/scripts/auth.js` отдает `200 application/javascript` и содержит новые exports. Но встроенный browser smoke Codex после deploy все ещё видит старое runtime-состояние (`typeof window.processOAuthCallback === 'undefined'` и те же ReferenceError в tab logs), поэтому bug пока оставлен в `In Progress`: нужен ручной fresh-browser QA Юрия, чтобы отделить уже внесённый кодовый фикс от edge/browser cache propagation. |
| BUG-2026-07-14-002 | Staging worker /analytics/lite-event отвечает 404 | Medium | P1 | Triaged | Codex | ANALYTICS-001 | Узкий API smoke 2026-07-14: fresh trial account успешно проходит `/auth/register`, `/auth/login`, `/auth/me`, `save/update/done task` и `/anthropic`, но `POST /analytics/lite-event` для action `plan-view`/`focus-open`/`statistics-open` трижды вернул `404`. До admin `/analytics/summary` smoke дело не доходит. |
...
| NEW-002 | Статистика: неактуальные данные / не открываются активные задачи | Medium | P2 | Ready for QA | Юрий + Codex | N/A | В `.tmp-4e-app-publish/index.html` `loadStats()` пересобран на реальных local выборках, empty states больше не выглядят как stale-cache, а `Смотреть все` у active/promises получил живые `onclick` |

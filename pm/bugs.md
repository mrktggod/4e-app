...
**Решение:** в `index.html` добавлена клиентская валидация email до отправки формы. При пустом или невалидном email показывается понятная ошибка и запрос не отправляется. Для ответа сервера показывается состояние успеха или ошибка без перехода на пустой экран. Auth/reset-экраны скрывают нижнюю навигацию, если пользователь не авторизован. 2026-06-26 повторно найдено: после toast "ссылка отправлена" пользователь снова попадал на пустой экран. Исправлено жёстче: добавлен отдельный `forgot-result-overlay` вне `#app`, который показывает логическое продолжение поверх любых экранов, с действиями "Вернуться ко входу" и "Отправить ещё раз".
...
| --- | --- | --- | --- | --- |
| BUG-2026-06-25-002 | Сброс пароля принимает некорректный email и переводит пользователя на пустой экран | Пустой email, `fff`, корректный формат с несуществующим/существующим пользователем; отсутствие пустого экрана и нижней навигации на auth/reset-flow | Юрий | Awaiting QA after 2026-06-26 confirmation overlay fix |

...
| BUG-2026-07-24-001 | После выбора времени напоминания закрытая карточка не показывает, что именно выбрано | Medium | P2 | Ready for QA / auto green | Codex | BACK-070 | Root cause: выбранное значение сохранялось только в скрытом `select` и active-состоянии открытого popover; закрытый trigger оставался обычным колокольчиком. Fix: trigger получает короткую видимую метку `15 мин` / `1 час` / `1 день`, active-state и полный `aria-label`; `none` убирает метку. `npm run smoke:back067-reminder` проверяет метку, opacity, aria, сохранённое значение, размеры и кликабельность всех вариантов в light/dark. Нужен один визуальный iPhone/TMA spot-check точного preview. |

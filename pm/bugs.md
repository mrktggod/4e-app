...
| BUG-2026-07-22-002 | Tag popup и клавиатура перекрывают карточку задачи в iOS TMA | High | P1 | Confirmed / screenshot evidence | Codex | N/A | При открытии добавления тега абсолютный input-bar и нативная подсказка `<datalist>` `Дом` закрывают title/metadata; нет видимой отмены, клавиатура оставляет мало рабочего пространства. Root cause area: `index.html:464`, `index.html:5829`, `styles/screens/tasks.less:1449`. Evidence: `docs/tasks/BUG-2026-07-22-task-detail-ios-regressions.md`. |
| BUG-2026-07-22-003 | Длинный тег/заголовок перекрывают срок и приоритет, текст обрезается | High | P1 | Confirmed / screenshot evidence | Codex | N/A | На iPhone длинный tag режется по одной букве вертикально, title заходит под правые cards, часть текста теряется внутри fixed-height hero. Root cause area: конфликт поздних absolute/clamp QA overrides around `styles/screens/tasks.less:2481` и фиксированной hero height. Evidence: `docs/tasks/BUG-2026-07-22-task-detail-ios-regressions.md`. |

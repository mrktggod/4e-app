...
| --- | --- | --- | --- | --- | --- | --- | --- |
| BUG-2026-07-14-001 | Staging web auth показывает «Нет соединения» после успешных /auth/register и /auth/login | High | P1 | Triaged | Codex | N/A | Headless smoke 2026-07-14: UI на `https://4-ai-staging.pages.dev/` после регистрации и входа остаётся на auth-экране с toast «Нет соединения», хотя прямой API на staging worker для тех же credentials возвращает `200` и token. Блокирует `ONBOARD-001`/`HOME-001` и почти весь UI QA на staging. |
...
| BUG-2026-07-14-003 | Staging /auth/telegram падает Worker exception 1101 | High | P1 | Triaged | Codex | N/A | API smoke 2026-07-14: `POST /auth/telegram` после успешного login на `restless-lab-d737-staging.shelckograff.workers.dev` возвращает Cloudflare `1101 Worker threw exception` вместо `ok/token`. Это ломает staging telegram-link path и мешает живому bot/auth smoke. |

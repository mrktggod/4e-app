...
| BUG-2026-07-14-004 | AI quick action «Статистика» в чате 4 возвращает validation error вместо ответа/экрана | Medium | P2 | Triaged | Codex | NEW-002 / HOME-001 | Ночной staging-smoke 2026-07-14: на экране AI-чата кнопка-подсказка `📊 Статистика` отправляет сообщение `Покажи статистику моих задач`, после чего вместо ответа 4 / перехода в stats показывается ошибка `messages.0.id: Extra inputs are not permitted`. Это уже не browser-артефакт: ошибка отрисована прямо в UI в ответ на реальный action. |
| BUG-2026-07-14-005 | Календарь не показывает дедлайн задачи даже после явного выбора дня | Medium | P2 | Triaged | Codex | NEW-021 | Ночной staging-smoke 2026-07-14: после создания задачи, которая на home и в focus-panel уже показывает мету `Работа · поставлена сегодня · дедлайн сегодня`, экран `Календарь` сначала рендерит `Нет событий`, а после явного клика по дню `14` переключается на `Дедлайны — 14 июл / Нет задач на этот день / Все дедлайны / Нет дедлайнов`. То есть UI знает выбранную дату, но не материализует уже видимый в home дедлайн в календарный список. |

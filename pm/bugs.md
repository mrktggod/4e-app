...
| --- | --- | --- | --- | --- | --- | --- | --- |
| BUG-2026-07-25-008 | Expired Premium: завершение задачи показывает общее `Ошибка сохранения` вместо понятного premium/paywall состояния | High | P1 | Triaged / queued for night | Codex | PREMIUM-UX-001 | Пользователь Миша тестировал с истёкшим Premium: при попытке завершить задачу UI показал только `Ошибка сохранения`. Ожидание: если backend возвращает premium/entitlement denial, интерфейс прямо объясняет, что действие недоступно без Premium, и ведёт на экран преимуществ/оплаты; не маскировать это как обычный save failure. Скрин: чат 2026-07-25, premium expired task detail. |

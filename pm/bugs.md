...
| BUG-2026-07-25-011 | Voice mode: кнопка `Отменить` и стрелка назад не выходят из режима | High | P1 | Triaged / queued for night | Codex | NEW-020 | В голосовом режиме пользователь не может выйти: `Отменить` и back arrow не срабатывают. Ожидание: оба контрола останавливают запись/распознавание, чистят transient state и возвращают на предыдущий безопасный экран. |

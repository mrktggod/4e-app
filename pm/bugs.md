...
| BUG-2026-07-22-001 | На iPhone/TMA не нажимается выбор времени уведомления в карточке задачи | High | P1 | Done | Codex | BACK-064 | Исправлено в app: hidden `<select>` не вложен в reminder `<button>`, trigger имеет 44x44 CSS px, popover расположен поверх карточки и его варианты не схлопываются. Алексей 2026-07-24 подтвердил на телефоне светлую и тёмную темы: popover нормально открывается и выбранное значение сохраняется; desktop также зелёный. `npm run smoke:back067-reminder` повторно прошёл на 390x844 в light/dark. Реальная доставка/звук/вибрация остаются отдельным `BACK-064`. |
| BUG-2026-07-24-001 | После выбора времени напоминания закрытая карточка не показывает, что именно выбрано | Medium | P2 | Done | Codex | BACK-070 | Root cause: выбранное значение сохранялось только в скрытом `select` и active-состоянии открытого popover; закрытый trigger оставался обычным колокольчиком. Fix: trigger получает короткую видимую метку `15 мин` / `1 час` / `1 день`, active-state и полный `aria-label`; `none` убирает метку. `npm run smoke:back067-reminder` проверил метку, opacity, aria, сохранённое значение, размеры и кликабельность всех вариантов в light/dark. Алексей 2026-07-24 подтвердил exact preview на iPhone/TMA: метка `15 мин` видна и не ломает карточку. |
...
| BUG-2026-07-21-006 | Уведомления плохо видны и иногда не срабатывают | High | P1 | NEED-YURI delivery QA | Codex / Yuri QA | BACK-064 | UI-блокер выбора времени закрыт: Алексей подтвердил reminder popover на iPhone/TMA в light/dark. Следующий честный gate — поставить ближайшее напоминание, свернуть Telegram и проверить доставку правильному пользователю, заметность, звук/вибрацию с учётом системных разрешений. `BACK-055` закрывал action-feed карточки, но не real delivery/salience. |

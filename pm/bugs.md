...
| BUG-2026-07-25-009 | Expired Premium: голосовой режим выглядит как зависание/поломка вместо явного premium gate | High | P1 | Triaged / queued for night | Codex | PREMIUM-UX-001 / NEW-020 | В голосовом режиме у пользователя без активного Premium экран остаётся в состоянии `Говори...`/pipeline, без понятного объяснения: нужно оплатить или это баг. Ожидание: voice entrypoint до записи проверяет доступ или корректно обрабатывает отказ и показывает понятный Premium-required state с переходом к преимуществам. |

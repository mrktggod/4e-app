...
| --- | --- | --- | --- | --- | --- | --- | --- |
| BUG-2026-07-22-001 | На iPhone/TMA не нажимается выбор времени уведомления в карточке задачи | High | P1 | Done | Codex | BACK-064 | Закрыто реальной приёмкой Алексея 2026-07-23 на `https://qa-reminder-popover.4-ai-staging.pages.dev/`: desktop и iPhone PASS, dark и light PASS, popup отображается нормально, выбранное значение сохраняется и остаётся активным при повторном открытии. Follow-up fix ограничил круглый размер прямыми action-кнопками, исправил overflow/stacking и вернул light trigger 44x44; smoke проверяет dark/light geometry и hit ownership. Отдельный неблокирующий UX-хвост о невидимом закрытом состоянии вынесен в `BUG-2026-07-23-001`. Report: `pm/outbox/REPORT-BUG-2026-07-22-001-reminder-popover-followup-2026-07-23.md`. |
...
| BUG-2026-07-23-001 | При закрытом popup не видно, установлено ли напоминание и какое время выбрано | Medium | P2 | Todo | Codex | BACK-070 | Алексей подтвердил, что reminder сохраняется и подсвечивается внутри popup, но снаружи колокольчик выглядит одинаково для `Без` и активного напоминания. Нужен спокойный видимый active-state без открытия меню: точка/ring или короткий badge, плюс доступное имя с выбранным временем. Не блокирует настройку и доставку уведомления. |
...
| BUG-2026-07-21-006 | Уведомления плохо видны и иногда не срабатывают | High | P1 | NEED-YURI | Codex / Yuri QA | BACK-064 | UI-блокер выбора времени закрыт реальной iPhone-приёмкой 2026-07-23 (`BUG-2026-07-22-001 Done`). Теперь можно продолжить настоящий Telegram/device delivery smoke: правильный пользователь, доставка в выбранное время, заметность, звук и вибрация там, где платформа позволяет. `BACK-055` закрывал action-feed карточки, но не real delivery/salience. |

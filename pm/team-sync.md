...
- Addendum: added `BUG-2026-07-25-019` and `020`, plus night briefs `72` and `73`, for calendar task clickability and restoring the agreed task-detail glass layout.
- Addendum: added `BUG-2026-07-25-021` and night brief `74` for Telegram Mini App task-chat suggested-action `Confirm` not applying.
...
- Branch: `feat/admin-tariff-api`.
- Root cause: `.ask-bar` animated keyboard padding, so Playwright could measure the AI-chat input before the final keyboard reserve was applied.
- Fix: `.ask-input-shell.ask-bar--keyboard-open` now disables the transition while keyboard reserve is active; rebuilt `styles.css` and `styles.min.css`.
- Evidence: focused chat keyboard spec passed 2/2; full `npm run qa:prebeta` passed with 20/20 Playwright tests plus home, accessibility, notifications, privacy and viral/share smokes.
- PM sync: `BRIEF-2026-07-24-52` is `DONE`; `BRIEF-2026-07-24-53-glass-home-focus-dashboard-package2` is the next `NEW` task.
- Note: pre-existing local changes in `index.html`, `scripts/auth.js` and QA screenshot assets were left untouched unless generated by the current test command.
...
- Result: notifications header, unread badge, filters, and list wrapper inline styles moved into `styles/screens/tasks.less`; CSS rebuilt.
- Evidence: encoding markers `106 -> 106`, `npm run smoke:back055` passed, UI guard reports inline style attrs `309 / 465`.

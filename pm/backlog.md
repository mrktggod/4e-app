...
| BACK-036 | Web fallback Telegram-входа не открывает Telegram | Bug/Auth | P1 | Codex | Ready for QA | Г1 | В `index.html` заменён `tg://resolve` на `https://t.me/Denzel89bot?start=...`; `openTelegramLink()` вызывается только с Telegram `initData`; нужен live smoke веб-версии |
...
| INFRA-004 | VK-версия на хостинге VK Mini Apps | Infra/Platform | P1 | Codex | In Progress | Г0 | Собрать vk-поверхность и задеплоить на статик-хостинг VK (`@vkontakte/vk-miniapps-deploy`, `vk-apps.com`) — раздаётся с инфраструктуры VK и переживает белые списки/блокировки. API-запросы остаются на `edge.4-ai.site` (Workers из РФ доступны). Решение Юрия 2026-07-05: «вылечить хотя бы VK-версию, остальное пока через VPN» |

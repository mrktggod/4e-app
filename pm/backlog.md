...
| BACK-055 | Уведомления как лента внимания | Product/UI | P2 | Юрий | Todo | Г1 | Экран `Уведомления` перестаёт быть архивом событий: вкладка `Дедлайны` переименована в `Горит`, карточки дают действия `К задаче / Готово / Отложить`, `Отложить` раскрывает `15 мин / 1 час / 3 часа / Завтра`, переход `К задаче` открывает конкретный `task-detail` по `taskId`, пустое состояние звучит как `Пока всё спокойно` |

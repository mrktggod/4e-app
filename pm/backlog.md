...
| SMART-004 | Лаконичная фиксация задач в группах | Product/Bot | P1 | Codex | In Progress | Г1 | Локальный bot-source в `<worker-repo-root>/src/bot/` уже переведён на однострочное подтверждение, мгновенное сохранение и `✏️/✕` через `delete-task`; нужен runtime deploy и ручной smoke в тестовой группе |
| SMART-001 | Ростер участников группового чата | Tech/Bot | P1 | Codex | In Progress | Г1 | Локальный worker/bot source уже хранит roster в D1 `chat_members` через `upsert/get/mark-chat-members-left`; миграция `0007_chat_members.sql` применена на staging, нужен bot runtime smoke в тестовой группе |
| SMART-002 | Исполнитель с TG ID в задаче | Product/Bot | P1 | Codex | In Progress | Г1 | Локальный bot source уже пишет `assigneeTgId`/`assigneeUsername` из entities, reply и roster-fuzzy, а analyzer получает список участников; staging worker обновлён, нужен runtime smoke и проверка задачи вида `@вася ...` |

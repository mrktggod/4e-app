...
| BACK-055 | Распил index.html: общее ядро + адаптеры TG/VK | Tech | P1 | Codex, поэтапно | In Progress | Г1–Г2 | Начат поэтапный распил: добавлен `scripts/platform-adapter.js` для Telegram/VK/web detection, VK Mini Apps adapter вынесен из хвоста `index.html` в `FourPlatform.initVkMiniAppAdapter()`, whitelist-сборка Workers Static Assets включает runtime script; дальше выносить auth helpers малыми шагами |

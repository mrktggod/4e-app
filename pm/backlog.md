...
| BACK-043 | Responsive UI redesign экрана профиля | Bug/UI | P2 | Юрий | Triaged | Г1 | Пересобрать форму профиля на mobile/desktop web без смены цветовой схемы и шрифта: единая сетка, гармоничные поля, статусы phone/email/telegram не конфликтуют с input, секции "Дата рождения" и "О себе" не прилипают, textarea и счётчик выглядят цельно |

...
| BACK-044 | Упростить детальную карточку задачи | Product/UI | P2 | Юрий | Triaged | Г1 | В `task-detail` описание должно быть под заголовком; вкладку "Описание" убрать; порядок полей: Срок → быстрые кнопки → Статус → Приоритет → Напоминание; строки "Направление" и "Человек" не показывать; проверить mobile/web без регрессии сохранения |
...
| BACK-047 | Вшить v2 auth/privacy routes в worker и убрать фронтовые fallback | Tech/Auth | P1 | Codex | In Progress | Г1 | Worker branch `feat/back-047-v2-auth-privacy`: staging+prod smoke зелёные для `/v2/auth/legacy-session`, `/auth/identities`, `/v2/privacy/settings`; фронтовый cleanup вынесен в отдельную ветку после live `200` |

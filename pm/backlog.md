...
| BACK-034 | Staging-контур: D1/KV staging, `wrangler --env staging`, тестовый бот, dev-Pages | Tech/Process | P1 | Codex | In Progress | Г1 | staging D1/KV и worker уже подняты, `dev`-ветка Mini App задеплоена на `4-ai-staging.pages.dev`; bot username в коде по умолчанию `Denzel89bot` и при необходимости переопределяется через `?bot=...`. Главный оставшийся blocker для AI smoke — staging `ANTHROPIC_KEY` и связанные secrets |

...
| BACK-050 | Accessibility baseline критических сценариев | Product/UI/QA | P1 | Юрий + Codex | In Progress | Г1 | Шаги auth/forms и status/toast подготовлены в `index.html`: login/register/forgot/reset получили labels, `aria-describedby`, `aria-invalid`, полевые ошибки, keyboard submit и видимый focus; `#toast` стал `role=status`/`aria-live=polite`, критические сообщения переключаются в `role=alert`/`assertive`; дальше dialog/focus/touch-targets |

...
- **Now** — закрывает beta gate: доступ, QA, доверие, первый экран, тестовые аккаунты, РФ-доступность, быстрый feedback loop.
- **Next** — усиливает "личный штаб дня" после стабилизации: AI-память v1, календарная концепция, действия с людьми, активация и слияние аккаунтов.
- **Later** — рост и монетизация после первых сигналов удержания: платежные live-smoke, рефералки, шеринги, календарный MVP, умные AI-действия.
- **Future** — платформенное расширение и дорогие интеграции после доказанного спроса.
- **Icebox** — идеи с высокой стоимостью, риском доверия или слабым доказательством спроса.
...
| INFRA-002 | Чек РФ-доступности без VPN в release-checklist | QA/Process | P1 | Codex (docs) + Алексей | Todo | Г0 | Все прод-URL проверяются из РФ-сети без VPN перед переключением хостинга/домена минимум с двух независимых точек/операторов; результат фиксируется в release-checklist/WORK_LOG |
| INFRA-003 | Приватизация реп — только после ухода фронта с GitHub Pages | Process | P1 | Юрий | Blocked by INFRA-001 | Г0 | Пока фронт живёт на `mrktggod.github.io` на Free-тарифе, репо остаётся публичным; private делать только когда `app.4-ai.site` на Workers станет продом и `github.io` останется редиректом |
...
| BACK-048 | Тестовые аккаунты разработчиков с полными правами | Tech/QA | P1 | Юрий / worker | Todo | Г1 | У Алексея, Юрия и QA/dev есть отдельные тестовые аккаунты в staging и, при необходимости, production; аккаунты получают Premium/full-access без оплаты через защищённый backend/admin-механизм; пароли и токены не попадают в git; smoke подтверждает вход, задачи, AI-чат, голос, подписку и платежные экраны |
...
| BACK-055 | Уведомления как лента внимания | Product/UI | P1 | Юрий | Todo | Г1 | Экран `Уведомления` перестаёт быть архивом событий: вкладка `Дедлайны` переименована в `Горит`, карточки дают действия `К задаче / Готово / Отложить`, `Отложить` раскрывает `15 мин / 1 час / 3 часа / Завтра`, переход `К задаче` открывает конкретный `task-detail` по `taskId`, пустое состояние звучит как `Пока всё спокойно`; усиливает ежедневное возвращение и готовит пользователя к платной ценности |
| BETA-001 | Закрытый тест на 5-10 пользователей | PM/QA/Product | P1 | Алексей + Юрий | Todo | Г1 | 5-10 людей прошли сценарий утром: вход, первый план дня, создание задачи, AI-чат/голос, уведомления; собраны P0/P1 баги, 3-5 продуктовых выводов и решение, что делать перед монетизацией |
| ANALYTICS-001 | Lite-аналитика активации перед тестом | Product/Data | P1 | Codex | Todo | Г1 | Минимальные события без тяжёлого дашборда: вход, создание задачи, открытие плана дня, завершение задачи, возврат на следующий день; хватает для решения, есть ли retention-сигнал |
| FEEDBACK-001 | Быстрый feedback loop внутри приложения | Product/QA | P1 | Codex / Юрий | Todo | Г1 | В приложении есть простой путь сообщить проблему или непонятный сценарий; feedback попадает в `pm/bugs.md`/командный канал без потери контекста |
...
| BACK-026 | Слияние аккаунтов с переносом задач | Product/Tech | P1 | Codex | Todo | Г1–Г2 | При входе через второй провайдер система находит дубль по telegramId/vkId/email, сливает задачи и plan, удаляет дубль; повторный вход не создаёт новый аккаунт; является предпосылкой VK/Yandex auth и рефералок |
| BACK-045 | Авторизация через сервисы РФ: VK ID + Яндекс ID | Product/Auth | P1 | Алексей + Юрий + Codex | Todo | Г1–Г2 | На экранах входа и регистрации есть понятные варианты входа через VK ID и Яндекс ID; реализация идёт после/в связке с BACK-026, чтобы вход не создавал дубли; сценарий покрыт smoke для web, VK Mini App и Telegram Mini App |
...
| CAL-001 | Концепция календаря 4 | Product/Strategy | P1 | Алексей + Юрий + Codex | Todo | Г1 | ТЗ: `docs/tasks/CAL-001-calendar-concept.md`. Согласована модель, как календарь связывает задачи, уведомления, AI-дашборд, AI-память, внешнюю занятость и privacy; утверждены этапы CAL-002/CAL-003 и границы MVP без автозаписи во внешние календари |
| ONBOARD-001 | Первый AI-план за 60 секунд | Growth/UX | P1 | Алексей + Юрий | Todo | Г1–Г2 | Новый пользователь без объяснений создаёт/надиктовывает 3 задачи и сразу видит первый план дня; это activation-механика перед монетизацией, не тарифная модель |
...
| ARCH-001 | Распил index.html: общее ядро + адаптеры TG/VK | Tech | P1 | Codex, поэтапно | Todo | Г1–Г2 | CSS/JS вынесены в модули, vk.html использует общее ядро; готовит MAX и сторы; снимает проблему кириллицы при правках. Новый ID введён, чтобы не конфликтовать с BACK-036 Telegram fallback |
...
| BACK-038 | Аналитика событий + D1/D7 retention | Tech/Product | P2 | Codex | Todo | Г1–Г2 | Полный слой аналитики после `ANALYTICS-001`: события, шеринг, D1/D7 retention и дашборд для оценки беты, виральности и готовности к платному трафику |
...
| OMNI-001 | Омниканальные поверхности 4 | Product/Platform | P2 | Алексей + Юрий + Codex | Todo | Г2–Г3 | ТЗ: `docs/tasks/OMNI-001-omnichannel-surfaces.md`. По горизонтам разложены Web/PWA, виджеты, share sheet, native voice, email/browser sources и командный слой; зафиксировано, что каналы усиливают "личный штаб дня", а не превращают продукт в generic task manager |
...
| SMART-013 | «Задача — это промпт»: AI-декомпозиция на этапы | Product/AI | P2 | Codex | Triaged | Г2 | ТЗ: `docs/tasks/SMART-013-ai-task-decomposition.md`. Кнопка «Разбить на этапы» в блоке чек-листа task-detail: AI предлагает 3-7 шагов, пользователь подтверждает, этапы сохраняются как обычный `checklist`; без отдельной сущности подзадач и без автозаписи |
...
| VIRAL-005 | Виральная упаковка onboarding wow-момента | Growth/UX | P2 | Мимо | Todo | Г2 | После `ONBOARD-001` упаковать первый AI-план в ролики/шеринг и связать с VIRAL-001; сама активация отслеживается в Next |
...
| PLAT-002 | RuStore: PWA-обёртка | Platform | P2 | Codex | Todo | Г2 | Standalone-режим без TG/VK SDK на домене 4-ai.site, публикация в RuStore; требует ARCH-001 |
...
| BUG-2026-07-05-003 (отрицательное время после 22:00) | BACK-056 (Now) | Todo | Нужно закрыть до закрытого теста, потому что это ломает доверие к первому экрану |

Непокрытых багов уровня backlog сейчас нет. Коллизия ID устранена 2026-07-08: архитектурный распил `index.html` получил новый ID **ARCH-001**, а **BACK-036** остаётся только за Telegram web fallback. Ранее устранённые дубликаты: вкладка «Выполнено» BACK-025 → **BACK-039**, Admin API BACK-027 → **BACK-040**.
...
| ICE-002 | Полноценные нативные iOS/Android сверх PWA/TWA | Mini App и PWA достаточно на старте; дорого поддерживать до доказанного спроса | После 10k пользователей или сильного платного спроса |
| ICE-003 | Proactive mode — 4 сам инициирует общение | Требует зрелой AI-логики | Горизонт 2 |
| ICE-004 | Фоновый сбор контекста с телефона | Риск доверия и модерации в сторах; можно делать только opt-in по одному источнику | После SMART-007, privacy-модели и отдельного решения Алексея |

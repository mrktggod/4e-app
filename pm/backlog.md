...
| BACK-035 | QA smoke по qa-checklist перед закрытым тестом | QA | P1 | Алексей | In Progress | Г1 | Чек-лист размечен на авто и ручные шаги: `api-smoke` и reset-password негативные кейсы вынесены в автоматизацию, добавлен короткий manual-shortlist для Алексея; до Ready for QA остаётся пройти только ручной список и отдельно починить reset-password в кейсе `fff` |
| BACK-048 | Тестовые аккаунты разработчиков с полными правами | Tech/QA | P1 | Юрий / worker | In Progress | Г1 | В `4e-worker` добавлен закрытый admin seed/delete контур для dev/test аккаунтов по email с `ADMIN_SECRET`: выставляет paid/full-access, seed-задачи и cleanup только для `devTestAccount`; до Ready for QA остаются staging deploy, реальный seed трёх аккаунтов и live smoke |

...
| BACK-055 | Распил index.html: общее ядро + адаптеры TG/VK | Tech | P1 | Codex, поэтапно | In Progress | Г1–Г2 | Начат поэтапный распил: добавлен `scripts/platform-adapter.js` для Telegram/VK/web detection, VK Mini Apps adapter вынесен в `FourPlatform.initVkMiniAppAdapter()`, Telegram start/return URL helpers, VK launch params и OAuth PKCE/state helpers вынесены в platform layer; дальше выносить auth/referral helpers малыми шагами |

...
| BACK-048 | Тестовые аккаунты разработчиков с полными правами | Tech/QA | P1 | Юрий / worker | Ready for QA | Г1 | Staging dev/test аккаунты `dev1.4e@example.com`, `dev2.4e@example.com`, `dev3.4e@example.com` засеяны через `/admin/dev-accounts/seed`: у всех `plan=paid`, по 5 seed-задач, API smoke login → `/auth/me` → `/tasks` прошёл |

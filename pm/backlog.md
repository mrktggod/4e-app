...
| BACK-034 | Staging-контур: D1/KV staging, `wrangler --env staging`, тестовый бот, dev-Pages | Tech/Process | P1 | Codex | Почти Done | Г1 | Контур поднят и используется: staging worker+D1/KV, тестовый бот (токен ротирован), секреты загружены, dev-Pages на 4-ai-staging.pages.dev (⚠️ pages.dev из РФ — только с VPN). Остаток: SMART-006 smoke (в очереди сессии 4) |
| BACK-035 | QA smoke по qa-checklist перед закрытым тестом | QA | P1 | Алексей | Todo | Г1 | Все сценарии qa-checklist пройдены на iOS и Android после фиксов BACK-021/024/027; + пункт РФ-доступности без VPN с двух операторов |
| BACK-041 | Bot-side web fallback TG-входа: `/start auth_*` → кнопка «Вернуться в 4» | Bug/Auth | P1 | Codex | Ready for QA | Г1 | Кнопка ведёт на `${APP_BASE_URL}/?telegram_start=<token>`, TTL 10 мин + одноразовость на worker; прод задеплоен; Алексей гоняет сквозной live smoke веб-входа |
| BACK-042 | App-side web fallback TG-входа (быв. дубликат BACK-036) | Bug/Auth | P1 | Алексей | Ready for QA | Г1 | Ветка `fix/telegram-login-web-fallback`: pending startToken, returnUrl, приём `?telegram_start=`, очистка URL после входа. Пара к BACK-041. ⚠️ Алексей ночью 05.07 завёл «BACK-042» под UI-баг нижней панели — коллизия, UI-баг перенумерован в BACK-046 |
| BACK-043 | Профиль: responsive redesign формы (BUG-2026-07-05-002) | Bug/UI | P2 | Codex (ветка `fix/profile-responsive-ui`) | Todo | Г1 | Полный responsive-редизайн формы профиля (не точечный сдвиг): сетка/отступы/группировка, статусы не налезают на input, textarea «О себе» + счётчик одним блоком, TG WebView не сломан, «Сохранить» не конфликтует с нижней навигацией. Цвета и шрифт не менять. `docs/tasks/BUG-2026-07-05-002_profile_mobile_layout.md` |
| BACK-044 | Упростить детальную карточку задачи | Product/UI | P2 | Codex (ветка `fix/task-detail-card-cleanup`) | Todo | Г1 | Описание под заголовок, вкладку «Описание» убрать, порядок: Срок → быстрые кнопки → Статус → Приоритет → Напоминание; «Направление» и «Человек» скрыть; сохранение всех полей не ломать. `docs/tasks/BACK-044-task-detail-card-cleanup.md` |
| BACK-045 | Авторизация через VK ID и Яндекс ID | Product/Auth | P2 | Юрий (OAuth-регистрация) + Codex | Отложено до после закрытого теста (beta freeze) | Г1–Г2 | Встроить в единую модель аккаунта (BACK-005/026), не плодить дубли; secrets только в wrangler/GitHub Secrets; показывать как доп. вариант входа; обновить privacy. `docs/tasks/BACK-045-russian-service-auth.md` |
| BACK-046 | Нижняя панель растягивается на ширину браузера в web (BUG-2026-07-05-001, быв. «BACK-042» Алексея) | Bug/UI | P2 | Codex (ветка `fix/bottom-nav-app-width`) | Todo | Г1 | Панель по ширине экрана приложения, не viewport; проверka: desktop 1440/1024, mobile 390, Home+Calendar+AI-chat+profile; не сломать mobile safe-area и скрытие nav при клавиатуре в AI-чате |
...
| SMART-006 | Профиль пользователя в контексте AI-чата | Product/AI | P1 | Codex | In Progress (код готов, smoke в сессии 4) | Г1 | Ветка `feat/smart-006-profile-context`: system prompt получает имя, локальное время, тариф, статистику, топ людей. Smoke через webhook-путь staging-бота |
| SMART-004 | Лаконичная фиксация задач в группах | Product/Bot | P1 | Codex | Ready for QA | Г1 | Однострочник `✓ Имя: задача — срок` + кнопки ✏️/✕, сохранение сразу; smoke против staging-worker пройден; ветка `feat/smart-004-bot-confirmation`, ждёт QA Лёхи в тестовой группе |
| SMART-001 | Ростер участников группового чата | Tech/Bot | P1 | Codex | Ready for QA | Г1 | D1 `chat_members` (миграция 0007), upsert из сообщений/reply/join-leave; баг left_chat_member пойман и исправлен на smoke; ветка `feat/smart-001-002-roster-assignee` |
| SMART-002 | Исполнитель с TG ID в задаче | Product/Bot | P1 | Codex | Ready for QA | Г1 | assigneeTgId/assigneeUsername из entities/reply/fuzzy по ростеру; ростер передаётся в промпт analyzer; проверены @username и reply-задача; связан с BACK-030 |
...
| BACK-038 | Аналитика событий + D1/D7 retention | Tech/Product | P2 | Codex | Ready for QA | Г1–Г2 | Staging deploy `49939d43-808a-4b86-b756-3834dafc396e`; smoke passed: login/task_create/task_complete/dashboard_open/task_share + `/analytics/summary` + CORS для `PUT`/`Authorization` |
...
| INFRA-001 | Фронт на Workers Static Assets + app.4-ai.site на Worker | Infra | P0 | Codex | Done | Г0 | Глобальный контур готов и смержен: run_worker_first, self-host vendor-ассетов (jsdelivr блокирован в РФ!), динамический 404 с диагностикой. Жёсткие белые списки РФ конфигурацией не побеждаются (QUIC/DPI) — см. решение 2026-07-05: app.4-ai.site = мир/сторы, TG — github.io, VK — хостинг VK |
| INFRA-002 | Чек РФ-доступности без VPN в release-checklist | QA/Process | P1 | Codex (docs) + Алексей | Todo | Г0 | Обязательный пункт: все прод-URL проверяются из РФ-сети без VPN перед любым переключением хостинга/домена. Проверять минимум с двух операторов (сеть Юрия под жёсткими ограничениями — нужна вторая точка, Алексей). Урок миграции 2026-07-05 |
| INFRA-004 | VK-версия на хостинг VK Mini Apps | Infra/Platform | P1 | Codex | Done | Г0 | Задеплоено и подтверждено: production `https://prod-app54636698-c3cd4413b138.pages-ac.vk-apps.com/index.html`, CORS на edge.4-ai.site проверен, телефонный smoke Юрия в приложении VK без VPN пройден 2026-07-05. VK-канал работает при любых белых списках |
...
Непокрытых багов нет. Коллизия ID устранена 2026-07-04: Later-дубликаты переименованы — вкладка «Выполнено» BACK-025 → **BACK-039**, Admin API BACK-027 → **BACK-040**. Номера BACK-025 (AI-дашборд) и BACK-027 (баги дашборда) остаются за Now-задачами, на которые ссылаются bugs.md и коммиты. Файлы задач `docs/tasks/BACK-025-completed-tasks-week.md` и `docs/tasks/BACK-027-admin-tariff-map.md` переименовать в BACK-039/BACK-040 при синхронизации (Codex).

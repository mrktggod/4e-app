...
| BACK-050 | Человекочитаемые значения «Направления» в карточке | Bug/UI | P3 | Codex | Todo | Г1 | В поле «Направление» светится сырое значение `outgoing` вместо «Исходящее / Я должен»; замапить значения на русские лейблы везде, где направление отображается. Замечание Юрия на live-smoke BACK-044 (06.07) |

...
| BACK-046 | Ограничить нижнюю панель шириной экрана приложения | Bug/UI | P2 | Codex | In Progress | Г1 | Ветка `fix/bottom-nav-app-width`: `bottom-nav-v2` и `global-nav` на desktop/web должны быть по ширине app-контейнера, а не viewport; mobile safe-area и скрытие nav при клавиатуре не должны сломаться |

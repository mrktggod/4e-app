...
| NEW-001 | Уведомления: утренний брифинг не сработал | Bug/UX | P2 | Юрий + Codex | Todo | Г2 | Проверить все переключатели, почему не срабатывает push/напоминание «Утренний брифинг» для разных профилей и режимов |
...
| NEW-006 | Safe Area: перекрытие нижней навигацией Telegram Mini Apps | Bug/UX | P1 | Юрий + Codex | Todo | Г2 | Исправить паддинги и позиционирование, чтобы нижняя панель Telegram Mini Apps не перекрывала контент |
...
| NEW-008 | AI-чат: поле ввода перекрывается меню | Bug/UX | P1 | Юрий + Codex | Todo | Г2 | Убрать перекрытие поля ввода AI-чата нижним меню/панелью, сохранить доступность send/attach |
...
| BACK-030 | Участники задачи + отправка задачи пользователю 4 | Product/Tech | P1 | Codex | Ready for QA | Г2 | Добавлять других пользователей 4 в задачу; связывать аккаунты; задача приходит выбранному пользователю в его 4; в detail-flow уже сохраняются structured assignee (`@username` / `ID`) и бейдж «пользователь 4», дальше нужен живой smoke |
...
| INFRA-005 | RU API proxy для VK Mini App через Yandex Cloud | Infra/Platform | P0 | Codex + Алексей | In Progress | Г0 | Шаг 1: подготовлены `infra/yandex-api-gateway/ru-proxy-openapi.yaml`, runbook и VK build-config через `VK_API_BASE_URL`; ручной blocker — folder-id/доступы Yandex Cloud и технический домен gateway для phone-smoke без VPN |

...
1. `ask-action-preview`: move generated ask action/clarification preview inline styles in `index.html:6150-6860` into `.ask-*` classes. Smoke needed: add/extend a focused ask composer headless check before cleanup or pair with a manual mobile composer check. Risk: medium.

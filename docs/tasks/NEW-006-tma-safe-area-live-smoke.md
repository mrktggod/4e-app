...
| --- | --- | --- | --- |
| NEW-006-QA-001 | Открыть Mini App в Telegram на телефоне | Нижняя навигация не перекрывает контент | Not run |
| NEW-006-QA-002 | Profile: доскроллить до последнего действия | Последняя кнопка видна и нажимается выше nav | Not run |
| NEW-006-QA-003 | Notifications/settings screen | Последний пункт не спрятан под nav | Not run |
| NEW-006-QA-004 | Task list with many tasks | Последняя карточка доступна, bottom padding достаточный | Not run |
| NEW-006-QA-005 | Chat/input screen без клавиатуры | Ask bar не конфликтует с bottom nav | Not run |
| NEW-006-QA-006 | Chat/input screen с открытой клавиатурой | Input остаётся видимым, nav/keyboard не перекрывают поле | Not run |
...
## 5. Live result — 2026-07-20

**Окружение:** Telegram Mini App на iPhone; точные модель и версия iOS не зафиксированы.

**Пройдено:**

- последняя карточка задачи полностью доступна над нижней навигацией;
- низ профиля не перекрыт;
- поле AI-чата видно без клавиатуры и с открытой клавиатурой;
- после закрытия клавиатуры лишний offset не замечен.

**Не проверено:** notifications/settings до последнего пункта, поворот/resize, Android.

**Отдельная находка:** переход в календарь меняет пиктограммы и назначение центральной кнопки. Это заведено как `BUG-2026-07-20-001`; safe-area дефект при этом не воспроизведён.

**Итог:** partial pass на iPhone; статус не повышать до полного cross-platform Done без оставшихся проверок и повторного smoke после навигационного фикса.

...
- composer виден без клавиатуры;
- input и send доступны с открытой клавиатурой;
- длинный текст набирается без ухода поля за экран;
- сообщение отправляется;
- после закрытия клавиатуры поле возвращается в нормальную позицию.

**Переходный сценарий:** после открытия календаря нижняя навигация меняет пиктограммы, а центральная кнопка по короткому нажатию открывает voice вместо текстового AI-чата. Это отдельный `BUG-2026-07-20-001`, а не повтор исходного keyboard-overlap.

**Итог:** keyboard-overlap не воспроизведён на iPhone, но полный сценарий `чат → календарь → чат` требует повторного smoke после исправления навигации; Android не проверен.

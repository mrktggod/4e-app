...
| OAuth PKCE/state helpers | `scripts/platform-adapter.js:41`, `63`, `68`, `78` define redirect URI, PKCE, remember and consume state helpers. | Original 2026-07-17 evidence did not prove that external `scripts/auth.js` could see the inline `PLATFORM`/`WORKER` constants. 2026-07-25 follow-up fixed the global bridge with `window.PLATFORM=PLATFORM` and `window.WORKER=WORKER`; local OAuth-start smoke confirmed VK ID and Yandex start endpoints return `authUrl`. Real provider callback remains `NEEDS-REAL`. |

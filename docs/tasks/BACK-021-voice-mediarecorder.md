...
**Обновление 2026-06-29:** Алексей прислал скрин с live-симптомом "Ошибка микрофона" на экране голосового режима. Проверка текущего checkout показала, что `main` снова использует `SpeechRecognition`: MediaRecorder-flow был добавлен в `70a051f` / `origin/feat/voice-mediarecorder`, но затем откатан коммитом `e970d33`. Перед новой правкой нужно выяснить у Юрия/Claude, почему был откат, и только после этого возвращать фикс в отдельной ветке `fix/voice-mediarecorder`.
